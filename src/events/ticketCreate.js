...
async function handleTicketCreate(interaction) {
    ...
    try {
        ...
        const Guild = (await import('../database/models/Guild.js')).default;
        ...
        if (guildConfig?.settings?.tickets?.categoryId) {
            ...
            if (!ticketCategory) {
                return await interaction.editReply({
                    content: '❌ La categoría de tickets configurada no existe. Contacta con un administrador para que use `/ticket-config categoria` y configure una nueva.',
                    ephemeral: true
                });
            }
        } else {
